...
function createLibrary(argv) {
    const createLibraryForAccountId = function (accountId) {
        const options = _getHTTPOptions(argv, 'POST', API_PATH_V5_LIBS);
...
function updateLibrary(argv) {
    getLibraryByName(argv.name, argv, function (foundLib) {
        ...
        const options = _getHTTPOptions(argv, 'PATCH', API_PATH_V5_LIBS + libId);
...
function createVersion(argv) {
    fs.readFile(argv.file, 'utf8', function (err, code) {
        ...
        getLibraryByName(argv.name, argv, function (foundLib) {
            const libId = foundLib.id;
            const options = _getHTTPOptions(argv, 'POST', API_PATH_V5_LIBS + libId + '/versions');
...
function updateVersion(argv) {
    getLibraryByName(argv.name, argv, function (foundLib) {
        ...
        const options = _getHTTPOptions(argv, 'PATCH', API_PATH_V5_LIBS + foundLib.id + '/versions/' + foundVersion.id);
