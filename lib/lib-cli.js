...
const API_PATH_V5_ACCOUNTS = '/v5/accounts?page[size]=100';
...
const argv = require('yargs')
    ...
    .option('k', {
        alias: 'key',
        desc: 'impCentral login key (can be obtained from the impCentral user profile page)',
        type: 'string',
        required: true
    })
    .option('production', {
...
function _getAPIServerURL(argv) {
    console.log(argv.production ? API_URL_HOST_PRODUCTION : API_URL_HOST_STAGING);
...
function listLibraries(argv) {
    _list(argv, function (libs, accessToken) {
        if (!libs) {
            console.log("There are no libraries on the account");
            return;
        }
        let found = false;
        ...
        if (!found) {
            console.log("No libraries with the specified name found");
        }
    });
}

async function retrieveAccountIdAndDo(argv, callback) {
    const options = await _getHTTPOptions(argv, 'GET', API_PATH_V5_ACCOUNTS);
